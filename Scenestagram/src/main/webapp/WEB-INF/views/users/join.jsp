...
<script type="text/javascript">
	...
	function phoneNumberCheck() {
       	if (!phone_number.value.match(/^[0-9]+$/)) {
          	phone_number_err.innerText ='숫자만 입력하셔야합니다.'
              return false
     	} 
       	else if (phone_number.value.length < 10 || phone_number.value.length > 11) {
       	phone_number_err.innerText = '전화번호 길이가 길거나 짧습니다.'
           return false
       } 
       else{
       	phone_number_err.innerText=''
       	  return true
       }
   }
...
<script type="text/javascript">
    ...
    const phone_number_err = document.querySelector('.phone_number_err')
    phone_number.onblur = phoneNumberCheck
